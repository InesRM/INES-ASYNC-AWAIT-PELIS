document.addEventListener("DOMContentLoaded", () => {
    ...
    if (elements.searchButton) {
      ...
      elements.searchButton.addEventListener("click", async () => {
        ...
        try {
          const response = await fetch(`../php/getPeliculas.php?nombre=${searchValue}`);
          if (!response.ok) {
    ...
    if (elements.selectCategorias) {
      ...
      elements.selectCategorias.addEventListener("change", async () => {
        ...
        if (category) {
          try {
            const response = await fetch(`../php/getFilmByCat.php?cat=${category}`);
    ...
    elements.resultadoElement.addEventListener("click", async (event) => {
      if (event.target.classList.contains("btn-primary")) {
        ...
        try {
          const response = await fetch(`../php/getPeliculas.php?nombre=${name}`);
    ...
    elements.randomButton.addEventListener("click", async () => {
      try {
        const response = await fetch("../php/getRandomFilm.php");
        const pelicula = await response.json();
    ...
    elements.sendButton.addEventListener("click", async () => {
      ...
      if (selectedRow) {
        ...
        try {
          const response = await fetch("../php/savePelicula.php", {
            method: "POST",
            headers: {
